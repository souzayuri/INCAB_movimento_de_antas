...
d3.csv('data/results_distance_join.csv', d3.autoType)
    ...
    .then(tapirs => {
        ...
        console.log("🐾 Tapir individuals in circular chart data:", Array.from(groupedTapirs.keys()));
...
function selectTapirInUI(individualName) {
    console.log("🎯 Selecting tapir in UI:", individualName);
    ...
    biomeIds.forEach(biomeId => {
        ...
        if (matchingOption) {
            ...
            console.log(`✅ Selected ${individualName} in ${biomeData[biomeId].name}`);
        }
    });
    
    if (!foundInDropdown) {
        console.log(`⚠️ Could not find ${individualName} in any dropdown`);
    }
}

// NEW FUNCTION: Sync map with tapir selection
function syncMapWithSelection(selectedIndividuals) {
    console.log("🔄 Syncing map with selection:", selectedIndividuals);
    console.log("📋 Previously selected:", previouslySelected);
    
    if (!csvDataLoaded || !map.getSource("movement-tracks")) {
        console.log("⚠️ Map not ready or data not loaded");
    ...
    if (selectedIndividuals.length === 0) {
        console.log("📍 No individuals selected, resetting view");
        previouslySelected = [];
    ...
    console.log("🔍 Will zoom to:", individualsToZoom);
    ...
    console.log("✅ Found", allSelectedTracks.length, "total matching tracks");
    console.log("🎯 Found", tracksToZoom.length, "tracks to zoom to");
    
    if (allSelectedTracks.length === 0) {
        console.log("⚠️ No matching tracks found for:", selectedIndividuals);
        return;
    }
...
const biomeLocations = [
    { name: "Mata Atlântica", coordinates: [-52.18, -22.5], class: "atlantic-forest", color: "#2E8B57" },  
...
d3.csv('data/df_merged_na_coord3.csv', d3.autoType).then(data => {
    ...
    console.log("🗺️ Map individuals (individual_local_identifier):", Array.from(groupedIndividuals.keys()));
    console.log("🔗 Name mapping created:", Array.from(window.tapirNameMapping.entries()));
